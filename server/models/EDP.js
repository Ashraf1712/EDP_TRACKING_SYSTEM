...
class EDP {
	static async createEDP({staffEmail}) {
		...
		try {
			...
			const edpData = await db
				.promise()
				.query(
					"INSERT INTO edp (edp_id,created_by,updated_by) VALUES (?,?,?)",
					[uniqueEDPID, staffEmail, staffEmail],
				);
			return edpData;
		} catch (error) {
	...
	static async getNonCompletedEDPByEmail({staffEmail}) {
		if (!staffEmail) {
			throw new Error("Something wrong with the data");
		}
		try {
			const [rows] = await db.promise().query(
                ...
                FROM 
                    ...
                    LEFT JOIN status ON edp.edp_id = status.edp_id
                WHERE 
                    edp.created_by = ? AND status.status != 'Completed'
            `,
				[staffEmail],
			);
	...
	static async updateEDPDetails(edpID, updatedData) {
        ...
        `,
            ...
            `,
				[
					updatedData.competencyAddress,
					updatedData.competencyCluster,
					updatedData.actionPlan,
					updatedData.intervention,
					updatedData.remarks,
					edpID,
				],
			);
            ...
            `,
				[
					updatedData.status,
					updatedData.dueDate,
					updatedData.date_agreement,
					updatedData.date_review,
					edpID,
				],
			);
